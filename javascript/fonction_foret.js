var canvas = document.querySelectorAll('canvas');//canvas[0] contient le premeir canvas et canvas[3] le second
		...
		function isNextMoveAllow(newPosX, newPosY) {//on regarde si avec la position de coordonnées (newPosX, newPosY) on est pas sur une mauvaise case
			...
			var tile = Math.ceil(centrePersoX / 16) + (62 * Math.floor(centrePersoY / 16));//on enregistre le numéro de la tuile sur lequelle se trouve le perso
			var ligne = Math.ceil(tile/62);//on cherche sur quelle ligne se trouve le perso
			var colonne = tile - ((ligne-1)*62);//on cherche sur quelle colonne se trouve le perso
		...
		$(function(){
		  ...
		  function compte(){
		     $('#countdown').html("<br/>Temps écoulé : " +seconde+ " secondes");
		     $('#score').html("<br/>Votre Score : " +score);
		     seconde++; 
		  }
		...
		var posX = 25, posY = 10;//position du personnage
		//var posX = 960, posY = 450;//position du personnage
		var nbrFrame = 0;
		var p1 = false, p2 = false, p3 = false, p4 = false, p5 = false, p6 = false, p7 = false, p8 = false, ralentit = false;
		...
		function drawPerso(direction, step) {
			...
			context[3].clearRect(0,0,992,512);//on efface le second canvas
		...
		function drawMap0() {//au passage, on créer une fonction pour dessiner la carte
			context[0].clearRect(0,0,992,512);
		...
		function drawMap1() {//au passage, on créer une fonction pour dessiner la carte
			context[1].clearRect(0,0,992,512);
		...
		sprite.onload = function() {
			chargerMap("../room/room_dark.js");
		}
		sprite.src = "../pictures/sprite/sprite3.png" ;
